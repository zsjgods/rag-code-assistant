...
│   │
│   ├── rag/
│   │   ├── chunker.py          # AST 代码切块（函数/类粒度+相邻上下文）
│   │   ├── embedder.py         # TF-IDF 向量化（可扩展为稠密嵌入）
│   │   ├── retriever.py        # 两阶段检索（粗排 TF-IDF → 精排 cosine）
│   │   └── indexer.py          # 索引构建、持久化与增量更新
│   │
...
AST 级代码切块（函数/类粒度，保留相邻函数名作为上下文摘要），TF-IDF 向量化，两阶段检索：

1. **粗排**：TF-IDF 关键词匹配 → Top-50 候选
2. **精排**：Cosine 相似度重排序 → Top-K 最终结果

支持索引持久化（pickle + JSON），可通过 `rag_index` 工具构建，`rag_search` 工具查询。
...
| `/hooks` | 查看钩子统计 |
| `/skills` | 列出已加载技能 |
| `/index [root]` | 构建 RAG 代码索引（默认当前目录，加 `--force` 重建） |
